...
DMDef = {
    ...
    "question8": "Let's do some bot specifics. How do you assist someone if they need help linking their premium perks?",
    "question9": "How would you disable rob across the whole server, but keep the bot commands working in other channels?",
    "question10": "A user lost coins randomly and he doesn't know how, how would you help them solve this issue?",
    "question11": "You are assisting people in support. Somebody asks you something about the bot which you are not sure the answer of. There does not seem to be another member of staff available. How are you going to handle the situation?",
    "question12": "How long have you used Dank Memer?",
    "question13": "Any additional comments for the moderation team? If you don't get a response after your answer, something went wrong with this process. Start over and make sure you're answering the questions correctly",
    "applicant_id": None,
    "accepter_id": None,
    "channel_id": None,
}
...
class Application(Cog):
    ...
    def __init__(self, bot):
        ...
        self.config.register_guild(**DMDef)
        self.antispam = {}
    ...
    async def accept(self, ctx: commands.Context, target: discord.Member):
        ...
        applicant = get(ctx.guild.roles, name="Applicant")
        role = get(ctx.guild.roles, name="Baby Moderator")
        pred = MessagePredicate.yes_or_no(ctx)
        if applicant in target.roles:
            await ctx.send(f"You sure you want to accept this user as Trial Moderator?")
            try:
                await self.bot.wait_for("message", timeout=30, check=pred)
            except asyncio.TimeoutError:
                return await ctx.send("Yes or no would've sufficed, smh")
            role_add = role
            ...
            await ctx.send(
                f"Uhhh looks like {target.mention} hasn't applied for anything. Seek help"
            )
    ...
    async def deny(self, ctx: commands.Context, target: discord.Member):
        ...
        if applicant in target.roles:
            ...
            if pred.result is True:
                ...
                try:
                    reason = await self.bot.wait_for("message", timeout=300, check=check)
                except asyncio.TimeoutError:
                    return await ctx.send("You took too long. Try again, please.")
                await target.send(
